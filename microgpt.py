...
# Tokenizer
uchars = sorted(set(''.join(docs))) # unique chars in the dataset becomes token_ids
BOS = len(uchars) # token id for Beginning of Sequence (BOS)
vocab_size = len(uchars) + 1 # total number of unique tokens + 1 (for BOS)
...
matrix = lambda nout, nin, std=0.02: [[Value(random.gauss(0, std)) for _ in range(nin)] for _ in range(nout)] # outer loop -> rows
...
def rmsnorm(x):
    ...
    scale = (ms + 1e-5) ** -0.5 # scale = 1 / sqrt(RMS)
