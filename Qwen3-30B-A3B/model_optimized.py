...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, mask, cos, sin):
        ...
        # Use PyTorch's scaled_dot_product_attention fused kernel for efficient attention computation
        # queries, keys, values: (b, num_heads, seq_len, head_dim)
        # mask: (seq_len, seq_len) or (b, num_heads, seq_len, seq_len)
        # PyTorch expects attn_mask as float (masked positions = -inf) or bool (masked positions = True)
        # We'll use the mask as is (bool upper-triangular mask)

        # Note: torch.nn.functional.scaled_dot_product_attention expects input shape (b, num_heads, seq_len, head_dim)
        # and mask shape (b, num_heads, seq_len, seq_len) or (seq_len, seq_len)
        context = torch.nn.functional.scaled_dot_product_attention(
            queries, keys, values, attn_mask=mask, dropout_p=0.0, is_causal=False
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg):
        ...
        self.norm1 = FusedRMSNorm(cfg["emb_dim"], eps=1e-6)
        self.norm2 = FusedRMSNorm(cfg["emb_dim"], eps=1e-6)
...
class Qwen3Model(nn.Module):
    def __init__(self, cfg):
        ...
        # Parameter sharing: tie output head weights with token embedding weights to save memory
        ...
        self.final_norm = FusedRMSNorm(cfg["emb_dim"])
        self.out_head = nn.Linear(cfg["emb_dim"], cfg["vocab_size"], bias=False, dtype=cfg["dtype"])

        # Tie weights: out_head weight is the same as tok_emb weight (transposed)
